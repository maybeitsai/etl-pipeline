...
def sample_df():
    ...
    df = pd.DataFrame(data).astype({
        "title": str, "price": float, "rating": float, "colors": 'Int64',
        "size": str, "gender": str, "image_url": str,
    })
    return df
...
def test_load_to_gsheets_success_worksheet_exists(mock_creds, mock_authorize, mock_prepare, sample_df):
    ...
    credentials_path = "fake_creds.json"; sheet_id = "fake_sheet_id"; worksheet_name = "TestData"
...
def test_load_to_gsheets_success_create_worksheet(mock_creds, mock_authorize, mock_prepare, sample_df):
    ...
    mock_prepare.return_value = prepared_data
    credentials_path = "fake_creds.json"; sheet_id = "fake_sheet_id"; worksheet_name = "NewSheet"
    result = load.load_to_gsheets(sample_df, credentials_path, sheet_id, worksheet_name)
    assert result is True
    mock_prepare.assert_called_once_with(sample_df)
    mock_spreadsheet.worksheet.assert_called_once_with(worksheet_name)
...
def test_load_to_gsheets_empty_df(mock_creds, mock_authorize, empty_df, caplog):
    credentials_path = "fake_creds.json"; sheet_id = "fake_sheet_id"; worksheet_name = "EmptySheet"
    caplog.set_level(logging.WARNING)
...
def test_load_to_gsheets_credentials_not_found(mock_creds, mock_authorize, sample_df, caplog):
    credentials_path = "nonexistent_creds.json"; sheet_id = "fake_sheet_id"; worksheet_name = "TestData"
    caplog.set_level(logging.ERROR)
    result = load.load_to_gsheets(sample_df, credentials_path, sheet_id, worksheet_name)
    assert result is False
    mock_creds.assert_called_once()
...
def test_load_to_gsheets_value_error(mock_prepare_data, mock_creds, mock_authorize, sample_df, caplog):
    mock_gc = MagicMock(); mock_authorize.return_value = mock_gc
    credentials_path = "fake_creds.json"; sheet_id = "fake_sheet_id"; worksheet_name = "TestData"
    caplog.set_level(logging.ERROR)
    result = load.load_to_gsheets(sample_df, credentials_path, sheet_id, worksheet_name)
    assert result is False
...
def test_get_postgres_schema_standard_types():
    ...
    df = pd.DataFrame({
        'col_str': ['a', 'b'], 'col_int': [1, 2], 'col_float': [1.1, 2.2],
        'col_bool': [True, False], 'col_dt_naive': pd.to_datetime(['2024-01-01', '2024-01-02']),
        'price': [10.5, 20.0]
    }).astype({
        'col_str': 'object', 'col_int': 'int64', 'col_float': 'float64',
        'col_bool': 'bool', 'price': 'float64', 'col_dt_naive': 'datetime64[ns]'
    })
    
    # Mock the SQL composition
    with patch('psycopg2.sql.SQL', side_effect=lambda x: x), \
         patch('psycopg2.sql.Identifier', side_effect=lambda x: f'"{x}"'), \
...
# Helper function for mock setup (already good)
...
def test_load_to_postgres_success_covers_final_commit_log_return(mock_execute_values, mock_get_schema, 
    ...
    mock_sql_obj.as_string.return_value = f"INSERT INTO {table_name} (col1, col2) VALUES %s"
    
    # Mock the sql.SQL to return our prepared mock
    with patch("psycopg2.sql.SQL", return_value=mock_sql_obj), \
    ...
    # Assert
    assert result is True, f"Function should return True on success. Logs: {caplog.text}"
    ...
    # Don't check the actual string - just make sure cursor and data tuples match
    args = mock_execute_values.call_args[0]
    assert args[0] is mock_cursor  # First arg is cursor
    assert len(args) >= 3  # Make sure there are at least 3 arguments
    
    # Check if the third argument (data tuples) is correct
    # We're checking if the actual data passed matches what we expect
    actual_data = args[2]
    assert len(actual_data) == len(expected_data_tuples), f"Expected {len(expected_data_tuples)} data tuples, got {len(actual_data)}"
    
    mock_conn.commit.assert_called_once()
    assert "Successfully loaded" in caplog.text and f"into PostgreSQL table '{table_name}'" in caplog.text
...
@patch("utils.load.execute_values")
# No need to patch as_string here as execute_values is not expected to be called
...
def test_load_to_postgres_execute_error(mock_get_schema, mock_connect, sample_df, mock_db_config, caplog):
    ...
    def execute_side_effect(query, *args):
        query_str = str(query) # Use str() for robust check
...
# FIX: Patch Composed.as_string here
@patch("utils.load.psycopg2.connect")
@patch("utils.load._get_postgres_schema")
@patch("utils.load.execute_values", side_effect=Psycopg2Error("Insert failed"))
@patch("psycopg2.sql.Composed.as_string") # Patch as_string
def test_load_to_postgres_execute_values_error(mock_as_string, mock_execute_values, mock_get_schema, mock_connect, sample_df, mock_db_config, caplog):
    ...
    assert result is False
    mock_get_schema.assert_called_once_with(sample_df)
    assert mock_as_string.call_count > 0 # Ensure as_string was called
    mock_execute_values.assert_called_once() # Now it should be called
    mock_conn.commit.assert_not_called()
    mock_conn.rollback.assert_called_once()
    mock_conn.close.assert_called_once()
...
def test_load_to_postgres_missing_db_config_key(mock_connect, sample_df, mock_db_config, caplog):
    ...
    incomplete_config = mock_db_config.copy(); missing_key = "password"
...
@patch("utils.load.psycopg2.connect")
@patch("utils.load._get_postgres_schema")
def test_load_to_postgres_unexpected_error_during_commit(mock_get_schema, mock_connect, 
                                                      sample_df, mock_db_config, caplog):
    """Tests handling of unexpected non-DB errors during commit."""
    mock_conn, mock_cursor = setup_mock_conn_cursor(mock_connect)
    mock_schema_sql = "col1 TEXT"  # Simple string instead of mock
    mock_get_schema.return_value = mock_schema_sql
    ...
    with patch("utils.load.execute_values") as mock_execute_values, \
         patch("psycopg2.sql.SQL", return_value=mock_sql_obj), \
         patch("psycopg2.sql.Identifier", side_effect=lambda x: x):
...
@patch("utils.load.psycopg2.connect")
@patch("utils.load._get_postgres_schema")
def test_load_to_postgres_unexpected_error_during_execute_values(mock_get_schema, mock_connect, 
                                                              sample_df, mock_db_config, caplog):
    ...
    with patch("utils.load.execute_values", 
              side_effect=Exception("Unexpected generic error during execute_values")) as mock_execute_values, \
         patch("psycopg2.sql.SQL", return_value=mock_sql_obj), \
         patch("psycopg2.sql.Identifier", side_effect=lambda x: x):
